...
include_once(app_path().'\controller\buscarController.php');

Route::post('buscar/grupos', function (Request $request) {
	$json = '';
	try {
		...
		$buscar->usuarioConectado=$rdo->payload;

	
		$json =$buscar->getGrupos($request->parametros);
		$json = json_encode(["rta"=>true,"payload"=>$json]);
