...
/*
----Creado----2020-07-16 16:15:02.4374696 -0300 -03 m=+0.866676601
*/
...
Route::get('geo_estados', function (Request $request) {
	$json = '';
	try {
		...
		$geo_estados->usuarioConectado=$rdo->payload;
...
Route::get('geo_estados/{id}', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
		...
		$geo_estados = new Geo_estadosController();
		$geo_estados->usuarioConectado=$rdo->payload;
		$json = json_encode($geo_estados->getByPrim($request->id));
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
...
Route::post('geo_estados', function (Request $request) {
	...
	try {
		...
		$geo_estados = new Geo_estadosController();
		$geo_estados->usuarioConectado=$rdo->payload;
		$json = $geo_estados->create($request->id,$request->estado,$request->idPais);
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
