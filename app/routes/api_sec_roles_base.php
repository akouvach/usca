...
/*
----Creado----2020-07-16 16:15:03.5614899 -0300 -03 m=+1.990696901
*/
...
Route::get('sec_roles', function (Request $request) {
	$json = '';
	try {
		...
		$sec_roles->usuarioConectado=$rdo->payload;
...
Route::get('sec_roles/{id}', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
		...
		$sec_roles = new Sec_rolesController();
		$sec_roles->usuarioConectado=$rdo->payload;
		$json = json_encode($sec_roles->getByPrim($request->id));
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
...
Route::post('sec_roles', function (Request $request) {
	...
	try {
		...
		$sec_roles = new Sec_rolesController();
		$sec_roles->usuarioConectado=$rdo->payload;
		$json = $sec_roles->create($request->id,$request->rol,$request->esAdminGlogal,$request->esAdminGrupo,$request->esAdminGeografico);
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
