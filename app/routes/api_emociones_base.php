...
include_once(app_path().'\controller\emocionesController.php');

Route::get('emociones', function (Request $request) {
	$json = '';
	try {
		...
		$json =json_encode($emociones->getAll());
...
Route::get('emociones/{id}', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
		...
		$emociones = new EmocionesController();
		$emociones->usuarioConectado=$rdo->payload;
		$json = json_encode($emociones->getByPrim($request->id));
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
...
Route::post('emociones', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
...
Route::put('emociones', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
...
Route::delete('emociones', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
		...
		$emociones = new EmocionesController();
		$emociones->usuarioConectado=$rdo->payload;
		$json = $emociones->delByPrim($request->id); 
