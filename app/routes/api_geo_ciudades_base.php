...
/*
----Creado----2020-07-16 16:15:02.2936847 -0300 -03 m=+0.722891701
*/
...
Route::get('geo_ciudades', function (Request $request) {
	$json = '';
	try {
		...
		$geo_ciudades->usuarioConectado=$rdo->payload;
...
Route::get('geo_ciudades/{id}', function (Request $request) {
	$json = '';
	try {
		$token = $request->header('authorization');
		if(is_null($token)){
		...
		$geo_ciudades = new Geo_ciudadesController();
		$geo_ciudades->usuarioConectado=$rdo->payload;
		$json = json_encode($geo_ciudades->getByPrim($request->id));
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
...
Route::post('geo_ciudades', function (Request $request) {
	...
	try {
		...
		$geo_ciudades = new Geo_ciudadesController();
		$geo_ciudades->usuarioConectado=$rdo->payload;
		$json = $geo_ciudades->create($request->id,$request->idEstado,$request->idPais);
		http_response_code(200);
	} catch (Exception $ex){
		$json = json_encode(["rta"=>false,"payload"=>utf8_encode($ex->getMessage())]);
		http_response_code(500);
